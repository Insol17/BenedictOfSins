...
public class WolfSmallAI : MonoBehaviour, IEnemyAI
{
    ...
    private void DecideNextAction()
    {
        ...
        if (r < 50) nextIntent = MakeIntent("공격+출혈1", 6);
        else if (r < 60) nextIntent = MakeIntent("공격+출혈2", 3);
        else if (r < 90) nextIntent = MakeIntent("공격+출혈1b", 5);
    ...
    public void PerformTurn()
    {
        ...
        {
            Debug.LogWarning("[WolfSmallAI] refs missing");
            return;
        }
        ...
        switch (nextIntent.skillName)
        {
            case "공격+출혈1":
                DoAttack(6);
                player.AddOrUpdateDebuff("Bleed", 1);
                break;
            case "공격+출혈2":
                DoAttack(3);
                player.AddOrUpdateDebuff("Bleed", 2);
                break;
            case "공격+출혈1b":
                DoAttack(5);
                player.AddOrUpdateDebuff("Bleed", 1);
