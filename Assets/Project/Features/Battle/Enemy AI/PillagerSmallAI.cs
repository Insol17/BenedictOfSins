...
public class PillagerSmallAI : MonoBehaviour, IEnemyAI
{
    ...
    private void DecideNextAction()
    {
        ...
        if (r < 50) nextIntent = MakeIntent("공격", 4);
        else if (r < 80) nextIntent = MakeIntent("약화", 0, EnemyIntentType.Debuff);
        else nextIntent = MakeIntent("강한 공격", 6);
    }
    ...
    public void PerformTurn()
    {
        ...
        {
            Debug.LogWarning("[PillagerSmallAI] refs missing");
            return;
        }

        switch (nextIntent.skillName)
        {
            case "공격": DoAttack(4); break;
            case "강한 공격": DoAttack(6); break;
            case "약화": player.AddOrUpdateDebuff("Weak", 1); break;
            default: DoAttack(4); break;
        }
    ...
    {
        var type = forceType ?? (name == "약화" ? EnemyIntentType.Debuff : EnemyIntentType.Attack);
